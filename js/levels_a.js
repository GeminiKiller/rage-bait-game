...
window.LEVELS_A = [
  ...
  //   200px behind, totally safe to wait it out). Step up 90px onto
  //   ledgeExit(800); hidden spike_b(880-910) pops on arrival. Exit(925,340).
  {
  ...
  //   with spikeB(720-760). Final trigger(790) shoots a wall arrow;
  //   stop, watch it, hop it. Exit(900).
  {
    name: "Pop Quiz",
    deathMsgs: [
  ...
  // ================= LEVEL 7 =================
  // SOLUTION: hop spike1(390-420). Enter corridor(480); ceilSpike reveals
  //   AND slams down (0-440 @450px/s, ~1s) - keep RUNNING, corridor is
  //   only 100px/0.33s wide so a moving player clears it long before it
  //   lands. Trigger(620) pops spikePS(750-790); hop it. Trigger(830)
  //   makes the exit bolt from (865,430) up to (925,340) on exitLedge
  //   (step-up 90). Chase it there.
  {
    ...
    objects: [
      ...
      { id: 'ceilSpike', type: 'hazard', variant: 'spikes', dir: 'down', x: 480, y: 0, w: 100, h: 40, hidden: true },
  ...
  //   (460) reveals blk2b[550,680] (flat 90px gap). Landing trigger pops
  //   spikeF(640-670); hop it. Step down 80px onto floorB[680,960].
  //   Trigger(720) shoots an arrow - stop, watch it, hop it. Exit(880).
  {
    name: "Leap of Faith",
    deathMsgs: [
    ...
    objects: [
      ...
      { type: 'trigger', x: 460, y: 260, w: 90, h: 140, once: true, delay: 0.1,
        actions: [ { do: 'reveal', target: 'blk2b' }, { do: 'shake' } ] },
  ...
  //   you back before it to redo the hop. Bridge b1-b3[300,700] (400px)
  //   collapses 1.8s after the "RUN!" msg - cross in 1.33s, 0.47s margin.
  //   Landing trigger(700) pops spikeQ(800-830); hop it. Trigger(870)
  //   shoots an arrow - stop, watch, hop. Exit(900).
  {
    name: "Floor It",
    deathMsgs: [
  ...
  // ================= LEVEL 10 =================
  // SOLUTION (finale, chains 5): jump 150px gap(<=170, tightest). Landing
  //   trigger pops spike1(490-520); hop it. Trigger(520) starts crusher
  //   (600-680 corridor, 80px @550px/s, true pin vs floor) - wait for it
  //   to retract, dash through (0.27s, far under its ~1.16s safe window).
  //   Trigger(700) pops spike3(790-820); hop it. Trigger(840) shoots an
  //   arrow - stop, watch, hop. Step up 80px; trigger drops a ceiling
  //   spike toward the final step (keep moving, it's a 0.75s fall vs your
  //   0.13s crossing). Exit(920,350).
  {
    ...
    objects: [
      ...
      { id: 'ceilFinal', type: 'hazard', variant: 'spikes', dir: 'down', x: 890, y: 0, w: 50, h: 40, hidden: true },
      ...
      { type: 'trigger', x: 840, y: 380, w: 40, h: 100, once: true, delay: 0.3,
